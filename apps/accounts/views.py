...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            mail_subject = 'Por favor active su cuenta en Neron'
    ...
    return render(request, 'accounts/register_02.html', context)
...
def login(request):
    ...
    return render(request, 'accounts/login_02.html')
...
def forgotPassword(request):
    if request.method == 'POST':
        ...
        if Account.objects.filter(email=email).exists():
            ...
            messages.success(request, 'Un email fue enviado a su email para resetear la contraseña')
            return redirect('login')
        else:
...
def resetpassword_validate(request, uidb64, token):
    ...
    if user is not None and default_token_generator.check_token(user, token):
        ...
        messages.success(request, 'Por favor resetee su contraseña')
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            ...
            user.save()
            messages.success(request, 'La contraseña se reseteó correctamente')
            return redirect('login')
        else:
